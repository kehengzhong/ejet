...
eJet是采用标准C语言开发、实现了HTTP/1.0和HTTP/1.1协议功能的Web服务器，eJet Web服务器是以adif库和ePump框架为底层支撑，构建的一个事件驱动模型、多线程、大并发连接的轻量级的高效Web服务器。eJet Web服务器的全部功能都封装成一个300K左右大小的动态库或静态库，可以嵌入到任何应用程序中，使其具备像Nginx服务器一样强大的Web功能，也可以使用一二百行代码来调用eJet库，轻易实现一个高性能的Web服务器。
