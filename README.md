...
* [四. eJet核心功能模块](#四-ejet核心功能模块)
    ...
    * [4.3 HTTP Script脚本](#43-http-script脚本)
        ...
        * [4.3.2 HTTP Script脚本类型](#432-http-script脚本类型)
        * [4.3.3 Script脚本嵌入位置](#432-script脚本嵌入位置)
        * [4.3.4 Script脚本范例](#433-script脚本范例)
        * [4.3.5 Script脚本语句](#434-script脚本语句)
            * [4.3.5.1 条件语句](#4341-条件语句)
            * [4.3.5.2 赋值语句](#4342-赋值语句)
            * [4.3.5.3 返回语句](#4343-返回语句)
            * [4.3.5.4 响应语句](#4344-响应语句)
            * [4.3.5.5 rewrite语句](#4345-rewrite语句)
            * [4.3.5.6 addReqHeader语句](#4346-addreqheader语句)
            * [4.3.5.7 addResHeader语句](#4347-addresheader语句)
            * [4.3.5.8 delReqHeader语句](#4348-delreqheader语句)
            * [4.3.5.9 delResHeader语句](#4349-delresheader语句)
            * [4.3.5.10 addResBody语句](#43410-addResBody语句)
            * [4.3.5.11 appendResBody语句](#43411-appendResBody语句)
            * [4.3.5.12 addFile2ResBody语句](#43412-addFile2ResBody语句)
            * [4.3.5.13 appendFile2ResBody语句](#43413-appendFile2ResBody语句)
            * [4.3.5.14 try_files 语句](#43414-try_files-语句)
            * [4.3.5.15 注释语句](#43415-注释语句)
...
HTTP Script脚本共有两种类型，一种是以关键词"script"为标识符的脚本，另一种是以关键词"reply_script"为标识符的脚本。

以"script"为标识符的脚本程序是在HTTP请求到达eJet服务器并进行HTTPMsg实例化过程中，被调用进行解释执行。

以"reply_script"为标识符的脚本程序则是在HTTP响应发送到客户端之前，被调用起来进行解释执行。

这样，可以利用"script"脚本程序，对客户端发起的HTTP请求头和请求体，进行检查、校验、或添加和修改等操作。可以利用"reply_script"脚本程序对返回给客户端的HTTP响应头和响应体，进行检查、校验、添加和修改等操作。
...
另外一种嵌入Script脚本程序的位置，是在JSon中的Key-Value对中，在Value里增加特殊闭合标签<script> Script Codes </script>或者<reply_script> Script Codes </reply_script>，在标签里面嵌入Script脚本代码，执行完代码后返回的内容，作为Key的值，这种方式使得JSon规范中Key的值可以动态地由Script脚本程序计算得来。在Listen、Host或Location的常量赋值中，Value内容可以是script脚本，如
```
...
对adif 基础库中的json.c文件做了修改扩展，使得Json对象都能支持script脚本定义的这几种语法，如果某个对象下有名称为script或者reply_script的数据项，就认为该数据项下的Value值为脚本内容。这就将名称script和reply_script作为Json的缺省常量名称了，使用时轻易不要使用script和reply_script作为变量名。
...
HTTP Script脚本程序中类型为"script"的解释执行，是在创建HTTPMsg实例并设置完DocURI后，开始执行资源位置实例化流程，在实例化过程中，分别执行HTTPListen的Script脚本、HTTPHost的Script脚本、HTTPLoc的Script脚本。

HTTP Script脚本程序中类型为"reply_script"的解释执行，是在HTTPMsg实例被返回给客户端之前，开始执行响应消息头和消息体的编码处理之前，分别执行HTTPListen的ReplyScript脚本、HTTPHost的ReplyScript脚本、HTTPLoc的ReplyScript脚本。

注意：HTTP ReplyScript脚本程序会处理eJet所有模式的HTTP响应消息，包括eJet充当Origin服务器、前向代理、反向代理、FastCGI转发等各种模式下返回的HTTP消息内容，在发送前，都会执行ReplyScript脚本来进行检查、校验、增删改等操作。
...
一个if语句中，可以允许有多个判断条件，通过 与操作（AND、&&）或 或操作（OR、||）来实现多个判断条件的 并且关系 或 或者关系。基本语法如下：
```
  if (判断条件1 && 判断条件2) { ... } else if (判断条件3 || 判断条件4) { ... } else { ... }
```
其中，并且关系符 && 也可以用 and或AND 来代替，或者关系符 || 也可以用 or或OR 来代替。

if语句中的判断条件可以允许一定程度的嵌套，通过小括号()来区隔多个组合判断条件，如下所示：
```
  if ( (判断条件1 && 判断条件2) || 判断条件3 || (判断条件4 AND 判断条件5) ) { ... }
```
...
* (p) 变量1 > 变量2，判断数值1是否大于数值2，两个变量值内容是整型或浮点型，比较其大小，如果为字符串型，则做字符串比较
* (q) 变量1 >= 变量2，判断数值1是否大于等于数值2，两个变量值内容是整型或浮点型，比较其大小，如果为字符串型，则做字符串比较
* (r) 变量1 < 变量2，判断数值1是否小于数值2，两个变量值内容是整型或浮点型，比较其大小，如果为字符串型，则做字符串比较
* (s) 变量1 <= 变量2，判断数值1是否小于等于数值2，两个变量值内容是整型或浮点型，比较其大小，如果为字符串型，则做字符串比较
...
在返回响应内容给客户端之前，可以通过脚本指令addResBody给响应体首部额外添加内容，如在HTML内容中添加一段JavaScript脚本程序。

添加的内容需要双引号包含住，内容中如果含有双引号时，需使用反斜杠escape，此外需要在引号内escape的字符包括：
换行符\n或\r
单引号'
制表符\t
正反斜杠 \ /
...
跟addResBody相对应，在返回响应内容给客户端之前，可以通过脚本指令addResBody给响应体尾部额外添加内容，如在HTML内容中添加一段JavaScript脚本程>
...
将当前资源位置Location下的文件内容添加到响应消息体的头部，文件名的路径中如果是以根目录/开始，那么文件名则相对于当前HTTP请求实例化的资源位置Location的根路径，如果文件名的路径中没有以根目录/开始，则文件名的路径是相对于当前HTTP请求的相对路径。

使用本指令，将指定文件的内容读取到响应体的头部，并返回给客户端。

文件名可以是组成文件名的字符串外，也可以是各种变量。
...
将当前资源位置Location下的文件内容添加到响应消息体的尾部，文件名的路径中如果是以根目录/开始，那么文件名则相对于当前HTTP
请求实例化的资源位置Location的根路径，如果文件名的路径中没有以根目录/开始，则文件名的路径是相对于当前HTTP请求的相对路径
。
 
使用本指令，将指定文件的内容读取到响应体的尾部，并返回给客户端。
